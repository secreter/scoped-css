 ...
 */
const parentCodeScoped = require('../lib/scope');
...
describe('Css normal selector', function() {
    ...
    it('tagName.class should be `parent tagName.class`', function() {
        ...
        expect(parentCodeScoped(css,scopeCode)).to.be.equal(`.${scopeCode} h2.title { color:#444; }`);
    });
    ...
    it('* should be `parent *`', function() {
        ...
        expect(parentCodeScoped(css,scopeCode)).to.be.equal(`.${scopeCode} * { color:#444; }`);
    });
});
...
describe('Css pseudo-class selector', function() {
    it('selector:pseudo-class should be `selector:pseudo-class`', function() {
        ...
        expect(parentCodeScoped(css,scopeCode)).to.be.equal(`.${scopeCode} .title:hover { color:#444; }`);
    });
    it('selector::pseudo-class should be `selector::pseudo-class`', function() {
        let css=`.title::after{
            color:#444;
        }`
        expect(parentCodeScoped(css,scopeCode)).to.be.equal(`.${scopeCode} .title::after { color:#444; }`);
    });
});
...
describe('Css attribute selector', function() {
    it('selector[attribute] should be `parent selector[attribute]`', function() {
        ...
        expect(parentCodeScoped(css,scopeCode)).to.be.equal(`.${scopeCode} .title[href^="https"] { color:#444; }`);
    });
    it('[attribute] should be `parent [attribute]`', function() {
        let css=`[href^="https"]{
            color:#444;
        }`
        expect(parentCodeScoped(css,scopeCode)).to.be.equal(`.${scopeCode} [href^="https"] { color:#444; }`);
    });
});
